...
float rect(vec2 st) {
    float a = 1.0;
    float b = 1.0;
    // a = step(0.1, st.x) * step(0.1, 1.0 - st.x);
    // b = step(0.1, st.y) * step(0.1, 1.0 - st.y);
    // a = length(st.y);
    // b = length(st.y);
    a = max(max(length(st.x), length(st.y)), max(1.0-length(st.x), 1.0-length(st.y)));
    return a*b;
}
...
void main(){
	...
	gl_FragColor = vec4(vec3(step(0.6,df)), 1.0);//vec4(mix(vec3(0.),vec3(1.),step(0.7,df)),1.0);
}
